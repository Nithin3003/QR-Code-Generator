...
const Header = ({ email, onLogout }: { email: string, onLogout: () => void }) => (
    <AppBar position="fixed" elevation={0} sx={{ bgcolor: "rgba(255,255,255,0.8)", backdropFilter: "blur(12px)", borderBottom: "1px solid #e0e0e0", color: "#1976d2" }}>
        <Toolbar sx={{ justifyContent: "space-between", minHeight: 64 }}>
            ...
            <Stack direction="row" spacing={2} alignItems="center">
                <Button href="/dashboard" component={Link} variant="text" sx={{ fontWeight: 600 }}>
                    Dashboard
                </Button>
                {email && (
                    <Box sx={{ cursor: 'pointer' }} onClick={onLogout} title={`Logout (${email})`}>
...
export default function StatsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <Box sx={{ minHeight: "100vh", bgcolor: "#f8f9fa", display: "flex", flexDirection: "column" }}>
            ...
            <Container maxWidth="lg" sx={{ pt: 15, pb: 10, flex: 1 }}>
            ...
            <Dialog open={loginOpen} disableEscapeKeyDown>
                <DialogTitle>Login to View Stats</DialogTitle>
                <DialogContent>
                ...
                <DialogActions>
                    <Button onClick={() => window.location.href = '/dashboard'} color="inherit">Cancel</Button>
                    <Button onClick={handleLogin} variant="contained">View Stats</Button>
                </DialogActions>
            </Dialog>
        </Box >
    );
